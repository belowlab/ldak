...
//open tagging file, read first 9 elements then skip to end
if((input=fopen(tagfile,"r"))==NULL)
{printf("Error opening %s\n\n",tagfile);exit(1);}
if(fscanf(input, "%s ", readstring)!=1)
{printf("Error reading Element 1 of Row 1 of %s\n\n", tagfile);exit(1);}
fprintf(output2, "%s", readstring);
for(q=1;q<9;q++)
{
if(fscanf(input, "%s ", readstring)!=1)
{printf("Error reading Element %d of Row 1 of %s\n\n", q+1, tagfile);exit(1);}
fprintf(output2, " %s", readstring);
}
...
{
if(parttype==0&&q<num_reds-1){fprintf(output2," Annotation_%d",q+1);}
if(parttype==0&&q==num_reds-1){fprintf(output2," Base");}
if(parttype==1){fprintf(output2," Partition_%d",q+1);}
}
fprintf(output2,"\n");

//now do each row in turm
for(j=0;j<count;j++)
{
