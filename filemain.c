...
int read_bimfile(char *bimfile, int *chr, char **preds, double *cm, double *bp, char *al1, char *al2, int length, int *keeppreds_use, int type, double window_cm)
...
if(found>0)	//check not smaller than previous one
{
...
if(window_cm!=-9999&&found>0)	//check cm consistent with previous one
{
...
if(fixn==-9999){cols[4]=find_head("n", sumsfile, count2);}
...
{
if(strcmp(readlist[cols[3]],"NA")==0){chisb[j]=-9999;}
...
{
if(strcmp(readlist[cols[5]],"NA")==0){chisb[j]=-9999;signsb[j]=-9999;}
...
if(chisb[j3]==-9999)	//was NA
{flag=1;}
else	//not NA
{
if((al1[j2]!=gotal1[j3]&&al1[j2]!=gotal2[j3])||(al2[j2]!=gotal1[j3]&&al2[j2]!=gotal2[j3]))	//incons
{
flag=1;
...
if(ccount>5||(acount>0&&amb==0)||pcount>5){printf("\n");}
...
if(count3==0){printf("\nError, %s contains (valid) summary statistics for none of the %d predictors\n\n", sumsfile, length);exit(1);}
if(count3<length)
{
if(type==0){printf("\nError, %s contains (valid) summary statistics for only %d of the %d predictors; either correct the file or exclude predictors without summary statistics\n\n", sumsfile, count3, length);exit(1);}
if(checksums==1)
{printf("\nError, %s contains (valid) summary statistics for only %d of the %d predictors; to continue despite this error, use \"--check-sums NO\"\n\n", sumsfile, count3, length);exit(1);}
printf("Warning, %s contains (valid) summary statistics for only %d of the %d predictors\n", sumsfile, count3, length);
}
else{printf("\nHave found valid summary statistics for all %d predictors\n", length);}
...
int read_tagfile(char *tagfile, char **spreds, char *sal1, char *sal2, double *stags, double **svars, double **ssums, int num_parts, int length, int parttype, char *altfile, char *bpredfile, char *cpredfile, int tagone)
{
int j, q, q2, count, count2, head;
int *predorder, *usedpreds, *indexer, *indexer2;
double sum, *avalues;

char readchar, readstring[500], **wantpreds, *rs;
...
if(parttype==0)	//using annotations - ssums[num_parts][num_parts] is total number of preditors
{
for(q=0;q<num_parts;q++){ssums[q][num_parts+1]=ssums[q][num_parts]/ssums[num_parts-1][num_parts];}
}
else	//using partitions - sum of ssums[][num_parts] is total number of preditors
{
...
{
if(indcount<ns)
{printf("%d out of %d samples have values for Phenotype %d (remainder will be set to mean)\n", indcount, ns, m+1);}
else
...
//have already checked all individuals present
...
(void)read_data_fly(datafile, dtype, datatemp, NULL, count2, indexer, 0, num_tops, tkeeppreds, datainputgz, 0, count, num_preds, genskip, genheaders, genprobs, missingvalue, -9999, -9999, nonsnp);
...
if(strcmp(causalsfile,"blank")!=0)	//read in and match against allpreds
{
...
for(j=0;j<num_causals;j++)
{
effects[m][revs[causals[m][j]]]=RNOR;
//tweaked here for correlated effects
...
value=RNOR;
value2=RNOR;
effects[m*2][revs[causals[m][j]]]=mat[0]*value+mat[2]*value2;
effects[m*2+1][revs[causals[m][j]]]=mat[1]*value+mat[3]*value2;
}
}
}	//end of bivar!=-9999
...
{
if(count2==length){printf("Error, all %d predictors are in exactly one annotation, you should be using \"--partition-number\" and \"--partition-prefix\" instead of \"--annotation-number\" and \"--annotation-prefix\"\n\n", length);exit(1);}
...
{printf("There are %d predictors in more than one partition; each predictor will be allocated fully to all partition it belongs (the alternative is to use \"--all-one\" NO, in which case a predictor in X categories will be allowed 1/X to each)\n\n", count-count2);}
}
...
else	//not adding extra partition, will squeeze down everything outside (unless not reducing)
{
