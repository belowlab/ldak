...
{
//will try m from -1.5 to .5 (201) and s from .01 to .5 (50)
num_means=201;
mvals=malloc(sizeof(double)*num_means);
for(j=0;j<num_means;j++){mvals[j]=-1.5+.01*j;}
num_sds=50;
...
{
//will try m from -1.5 to .5 (101) and s from .01 to .1 (10)
num_means=101;
mvals=malloc(sizeof(double)*num_means);
for(j=0;j<num_means;j++){mvals[j]=-1.5+.02*j;}
num_sds=10;
svals=malloc(sizeof(double)*num_sds);
for(k=0;k<num_sds;k++){svals[k]=.01+.01*k;}
