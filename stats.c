...
current=read_data_fly(datafile, dtype, data, ps, num_samples_use, keepsamps, bitstart, bitend, keeppreds_use, datainputgz, current, num_samples, num_preds, genskip, genheaders, genprobs, missingvalue, -9999, -9999, nonsnp);
...
{
if(data[(size_t)j*num_samples_use+i]!=missingvalue){sum+=data[(size_t)j*num_samples_use+i];indcount++;}
}
