...
void fast_reml(int ns, int num_covars, int num_tops, int num_envs, int num_regs, double* Y, double* Z, double* X, int Xtotal, int *Xstarts, int *Xends, double *Xsums, double *Xnss, double *Xrhos, double prev, int np, int compact, int constrain, double shrink, double strip, double tol, int maxiter, char *outfile, char **ids1, char **ids2, int num_preds, char **allpreds, char *allal1, char *allal2, int *rkeeppreds, int **regindex, double *rcentres, double *rmults, double *rweights, int *tkeeppreds, double *tcentres)
{
...
double *AI, *AI2, *AI3, *AIsave, *BI, *BIsave, *J, *JAI, *JAIJT, *JAIJTsave;
...
JAIJTsave=malloc(sizeof(double)*total*total);
...
//save AI and BI in case returning
for(r=0;r<total;r++)
{
BIsave[r]=BI[r];
for(r2=0;r2<total;r2++){AIsave[r+r2*total]=AI[r+r2*total];}
}
}
else	//recover saved derivatives
{
for(r=0;r<total;r++)
{
BI[r]=BIsave[r];
for(r2=0;r2<total;r2++){AI[r+r2*total]=AIsave[r+r2*total];}
}
}
...
for(r=0;r<total;r++)
{
for(r2=0;r2<total;r2++){JAIJTsave[r+r2*total]=JAIJT[r+r2*total];}
}
...
{
for(r2=1;r2<total;r2++)
{fprintf(output10, "%.6f\t", JAIJTsave[r+r2*total]);}
fprintf(output10, "\n");
}
fclose(output10);
}	//end of compact=0
