...
void write_scores(double **guesses, int **nums, int ns, int num_scores, char **ids1, char **ids2, char *outfile, double *resp, double missingvalue)
{
int i, r;
...
for(r=0;r<num_scores;r++)
{
if(nums==NULL){fprintf(output, "%.6f\tNA\t", guesses[r][i]);}
else{fprintf(output, "%.6f\t%d\t", guesses[r][i], nums[r][i]);}
}
fprintf(output, "\n");
}
fclose(output);
