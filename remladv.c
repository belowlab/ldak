...
//will have null model (num_regs=0), else normal (stats!=NULL) or permutation (stats==NULL)
...
{
int i, j, j2, k, k2, r, count, token, one=1, info;
double sum, sumsq, max, alpha, beta;

int total, rflag, *fixed, *fixedsave, nlost, nlost2;
...
int Xstarts[1], Xends[1], num_kins=1, shortcut=1, constrain=1, memsave=-9999;
double Xsums[1], *V=NULL, *V2=NULL, *V3=NULL, *VZ=NULL, *VZZTVZ=NULL, *P=NULL, **mkins=NULL;
...
free(AI);free(AI2);free(AI3);free(AIsave);free(BI);free(BIsave);free(J);free(JAI);free(JAIJT);
