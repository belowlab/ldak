...
//ridge regression - when changing this, consider changing also boltbayesr.c

///////////////////////////

//sum-her - can only have at most one of catfile, taufile or divide!=-9999
...
{svars[q]=malloc(sizeof(double)*count);ssums[q]=malloc(sizeof(double)*(num_parts+2));}

//read tagfile - if extracting, count will be reset
count=read_tagfile(tagfile, spreds, sal1, sal2, stags, svars, ssums, num_parts, count, parttype, altfile, bpredfile, cpredfile, tagone);
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename2);exit(1);}
fprintf(output2,"Category Trait1_Her Trait1_SD Trait2_Her Trait2_SD Both_Coher Both_SD Correlation SD\n");
for(q=0;q<num_parts;q++)
{
if(parttype==0&&q<num_parts-1){fprintf(output2,"Annotation%d ",q+1);}
if(parttype==0&&q==num_parts-1){fprintf(output2,"Base ");}
if(parttype==1){fprintf(output2,"Partition%d ",q+1);}
