...
int find_strings(char **stra, int lengtha, char **strb, int lengthb, int *indexa, int *indexb, char *filenamea, char *filenameb, int *preordera, int *preorderb, int type)
...
{
if(preordera!=NULL)	//order provided
{
for(a=0;a<lengtha;a++){ordera[a]=preordera[a];}
}
else
{
...
{
if(preorderb!=NULL)	//order provided
{
for(b=0;b<lengthb;b++){orderb[b]=preorderb[b];}
}
else
{
