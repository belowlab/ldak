...
//Some kinship functions requested by others
...
read_kins(kinstems[0], NULL, kins_single, 1.0, num_samples_use, ids3, 0);
printf("\n");
...
if(count==0){printf("Error, there were no off-diagonal allelic correlations less than %.6f\n\n", cutoff);exit(1);}
printf("There were %d off-diagonal allelic correlations less than %.6f\n\n", count, cutoff);
...
printf("Kinship matrix saved with stems %s\n\n", filename);
...
{
kin_warn(2, num_samples_use, 0, 1);
kins_single=malloc(sizeof(float)*num_samples_use*num_samples_use);
...
{
anal_warn(num_samples_use, num_samples_use);
ZZT=malloc(sizeof(double)*num_samples_use*num_samples_use);
}
else
{
...
if(mode==169)	//just one kinship, so get ZZT, then mulitply by kin and save
{
printf("Constructing the kinship matrix\n\n");

alpha=1.0;beta=0.0;
dgemm_("N", "T", &num_samples_use, &num_samples_use, &num_fixed, &alpha, Z, &num_samples_use, Z, &num_samples_use, &beta, ZZT, &num_samples_use);
for(i2=0;i2<num_samples_use;i2++)
{
for(i=0;i<num_samples_use;i++){kins_single[(size_t)i2*num_samples_use+i]*=ZZT[(size_t)i2*num_samples_use+i];}
}
...
free(kins_single);
free(Z);
if(mode==169){free(ZZT);}
else{free(kins_single2);}
}
