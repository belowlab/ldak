...
void read_bed_full(char *bedfile, unsigned char **data_char, int num_samples_use, int *keepsamps, int length, int *keeppreds, int num_samples, int num_preds)
{
...
for(j=0;j<length;j++)
{
if(j%20000==0){printf("Reading predictor %d out of %d\n", j+1, length);}
...
{printf("Error reading %s; does not contain %d samples and %d predictors\n\n", spedfile, num_samples, num_preds);exit(1);}
fseeko(input, 0, SEEK_SET);
current=0;
...
void read_speed_full(char *speedfile, float *speedstarts, float *speedscales, unsigned char **data_char, unsigned short **data_short, int num_samples_use, int *keepsamps, int length, int *keeppreds, int num_samples, int num_preds, int nonsnp)
{
int i, j, current;
...
speedscales[j]=(maxfloat-minfloat)/254;

if(size==1)
{
if(fread(rowchars, sizeof(unsigned char), num_samples, input)!=num_samples)
{printf("Error reading values for Predictor %d from %s\n\n", keeppreds[j]+1, speedfile);exit(1);}
for(i=0;i<num_samples_use;i++){data_char[j][i]=rowchars[keepsamps[i]];}
}
else
{
if(fread(rowshorts, sizeof(unsigned short), num_samples, input)!=num_samples)
{printf("Error reading values for Predictor %d from %s\n\n", keeppreds[j]+1, speedfile);exit(1);}
for(i=0;i<num_samples_use;i++){data_short[j][i]=rowshorts[keepsamps[i]];}
}
