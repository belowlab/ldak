...
//now set num_samples and allids3 to intersect / union
num_samples=Xnall[0];
...
if(num_files>1)
{
if(comsamps==1){printf("There are %d samples common to the %d datasets\n", num_samples, num_files);}
else{printf("There are %d samples across the %d datasets\n", num_samples, num_files);}
}
...
if(Xnuse[k]==0){printf("Error, will not be using any samples from Dataset %d\n\n", k+1);exit(1);}

for(i=0;i<Xnuse[k];i++)
{
i2=Xks2[k][i];
if(usedids[i2]==0)	//have not already got details for this sample
{copy_string(ids1,i2,Xids1[k][Xks[k][i]]);copy_string(ids2,i2,Xids2[k][Xks[k][i]]);usedids[i2]=1;count++;}
}
}
...
read_bimfile(bimstems[k], Xchr[k], Xpreds[k], Xcm[k], Xbp[k], Xal1[k], Xal2[k], XNall[k], NULL, 1, -9999);
}
...
count3=0;for(j=0;j<XNall[k];j++){count3+=(Xbp[k][j]>0);}
if(count3==0)
{printf("Error, after filtering predictors, none of those in %s remain\n\n", bimstems[k]);exit(1);}
printf("\n");
}
...
if(Xbp[k][j]>0)
{
if(Xchr[k][j]==Xchr[k][found]&&Xbp[k][j]==Xbp[k][found])	//matches - remove j, found stays same
{
if(exdups==1){Xbp[k][j]=-1;}
count++;flag=1;
}
...
//now set num_preds and chr/preds/bp/al1/al2 to intersect / union - will sort cm later

num_preds_use=0;
for(j=0;j<XNall[0];j++){num_preds_use+=(Xbp[0][j]>0);}
...
{
if(usenames==0)	//match based on position - must first save predictor details
{
count=num_preds_use;
...
else	//match based on names (must have compreds=1) - know that names in Xpreds are unique
{
indexer=malloc(sizeof(int)*num_preds_use);
indexer2=malloc(sizeof(int)*num_preds_use);
count=find_strings(preds, num_preds_use, Xpreds[k], XNall[k], indexer, indexer2, NULL, NULL, NULL, NULL, 3);
if(count==0)
{printf("Error, there are no predictors common to the first %d datasets\n\n", k+1);exit(1);}

//load up, checking positions and alleles
count2=0;
for(j=0;j<count;j++)
{
j2=indexer[j];
j3=indexer[j];
if(Xbp[k][j3]>0)	//bp will have only positive values
{
if(chr[j2]!=Xchr[k][j3]||bp[j2]!=Xbp[k][j3])
{printf("Error, the position of Predictor %s in Dataset 1 (%d:%.2f) does not match that in Dataset %d (%d:%.2f)\n", preds[j2], chr[j2], bp[j2], k+1, Xchr[k][j3], Xbp[k][j3]);}

if((al1[j2]!=Xal1[k][j3]&&al1[j2]!=Xal2[k][j3])||(al2[j2]!=Xal1[k][j3]&&al2[j2]!=Xal2[k][j3]))
{printf("Error, the alleles of Predictor %s in Dataset 1 (%c and %c) do not match those in Dataset %d (%c and %c)\n", preds[j2], al1[j2], al2[j2], k+1, Xal1[k][j3], Xal2[k][j3]);}
printf("\n");exit(1);

if(count2!=j2)	//squeeze down
{
chr[count2]=chr[j2];bp[count2]=bp[j2];
al1[count2]=al1[j2];al2[count2]=al2[j2];
...
for(j=count;j<num_preds_use;j++){free(preds[j]);}
num_preds_use=count;
...
if(usenames==0)	//match up by position
{
count=0;found=0;
for(j=0;j<XNall[k];j++)
{
...
else	//match up by name
{
XNuse[k]=find_strings(Xpreds[k], XNall[k], preds, num_preds_use, Xkp[k], Xkp2[k], NULL, NULL, NULL, NULL, 2);
if(XNuse[k]!=num_preds_use){printf("Doug error JK144 %d %d\n\n", XNuse[k], num_preds_use);exit(1);}
}

for(j=0;j<XNall[k];j++){free(Xpreds[k][j]);}free(Xpreds[k]);
free(Xchr[k]);free(Xbp[k]);
}	//end of k loop
...
{
if(genprobs>1&&mode!=190)
{Xcurrent[k]=read_data_fly(datastems[k], dtype, data2, ps2, Xnuse[k], Xks[k], bitstart2, bitend2, Xkp[k], Xdatainputgz[k], Xcurrent[k], Xnall[k], XNall[k], genskip, genheaders, genprobs, missingvalue, threshold, minprob, nonsnp);}
else
{Xcurrent[k]=read_data_fly(datastems[k], dtype, data2, NULL, Xnuse[k], Xks[k], bitstart2, bitend2, Xkp[k], Xdatainputgz[k], Xcurrent[k], Xnall[k], XNall[k], genskip, genheaders, genprobs, missingvalue, threshold, minprob, nonsnp);}

for(j2=0;j2<bitlength2;j2++)
{
j=Xkp2[k][bitstart2+j2]-bitstart;

//see if predictor needs turning
if(Xal1[k][Xkp[k][bitstart2+j2]]!=al1[bitstart+j])
{
...
j=Xkp2[k][bitstart2+j2];
fprintf(output2, "%s\t%c\t%c\t%d\t", preds[j], al1[j], al2[j], indcount);
if(indcount>0)
{
...
//see if changing coding - and maybe change alleles
