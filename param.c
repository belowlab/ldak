...
if(mpheno==-1)
{
printf("Will analyse each of the %d phenotypes in turn", num_resps);
if(compact==0){printf("; if you add \"--compact YES\", only the most important output files will be saved");}
printf("\n\n");
}
...
//say that can reduce repetitions

if(mode==128)
{
printf("Performing fast (but approximate) Haseman Elston Regression ");
if(num_parts>0)
{
if(parttype==1)
{
...
if(mode==129)
{
printf("Performing fast (but approximate) PCGC Regression ");
if(num_parts>0)
{
if(parttype==1)
{
...
if(strip!=0)
{printf("Will remove a proproption %.2f of the variation of each gene/chunk correlation matrix (to change this value use \"--strip\")\n\n", strip);}
if(strcmp(sumsfile,"blank")!=0&&shrink==1&&strip==0){printf("Consider using \"--shrink\" or \"--strip\" in an attempt to reduce the noise in each gene/chunk correlation matrix\n\n");}
...
if(savemat==0){printf("Add \"--save-matrix YES\" to also compute a heritability matrix (this is required if you plan to use \"--calc-ind-hers\" to estimate the per-predictor heritabilities)\n\n");}
}
...
if(mode==148)
{
printf("Calculating estimates of the heritability contributed by each predictor\n\n");
}
...
if(cvprop!=-9999||strcmp(bvsfile,"blank")!=0){printf(", using cross-validation to decide which is best");}
printf("; to instead consider 125 combinations, use \"--full-space YES\"\n\n");
}
else
{
printf("Will consider 125 combinations for (pi1, pi2, pi3), the fractions corresponding to the three largest variance terms");
if(cvprop!=-9999||strcmp(bvsfile,"blank")!=0){printf(", using cross-validation to decide which is best\n\n");}
}
...
if(mode==171)
{
printf("Calculating predictor statistics\n\n");
}
...
{printf("Will retain only samples and predictors present in all datasets\n");}
...
if(MKL==1){printf("When using the fast version of LDAK, use \"--max-threads\" to control the number of CPUs\n\n");}
