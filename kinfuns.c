...
int manip_kins(char *outfile, int num_kins, char **kinstems, double *kinsums, char **ids1, char **ids2, char **ids3, int ns, int kingz, int kinraw, int type, double *scales)
//type=0 - join-kins, type=1 - add, type=2 - subtract, type=3 - solve null
{
int i, i2, j, j2, k, count, count2, count3, flag, samedata;
double value;
...
if(flag==0)	//check name matches previous
{
...
//get first kins
kin_warn(1, ns, 0, 1);
kins_single=malloc(sizeof(float)*ns*ns);
value=1;if(scales!=NULL){value=scales[0];}
if(value!=0){read_kins(kinstems[0], NULL, kins_single, kinsums[0]*value, ns, ids3, 0);}
else
{
for(i=0;i<ns;i++)
{
for(i2=0;i2<ns;i2++){kins_single[(size_t)i*ns+i2]=0;}
}
}
...
if(value!=0)
{
if(type==0||type==1||type==3){read_kins(kinstems[k], NULL, kins_single, kinsums[k]*value, ns, ids3, 1);}
else{read_kins(kinstems[k], NULL, kins_single, -kinsums[k]*value, ns, ids3, 1);}
}
}
printf("\n");
