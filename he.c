...
double *hers, *hersds, *shares, *sharesds, *cohers, *jacks, stats[4];
...
like=-.5*stotal*(1+log(2*M_PI*Ysumsq/stotal));
...
for(k=0;k<total;k++){Ysumsqa-=DTSa[k]*hers[k];}
...
for(k=0;k<total;k++){Ysumsqb-=DTSb[k]*hers[k];}
...
if(total>0)
{
free(shares);free(sharesds);free(cohers);
